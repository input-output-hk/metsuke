...
Samples must survive server downtime, client restarts, and upgrades. Durability
could live client-side, server-side, or both; two spools mean two failure modes
and an ambiguous ACK.

## Decision

The client spools samples in local SQLite and deletes rows only on server ACK,
