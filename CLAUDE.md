...
- Limits (sizes, rates, intervals) are configuration, never constants. A limit is one where changing the value changes what is accepted, stored, sent or refused. A number that only stages work or shapes how it is reported, a copy granularity or a log line's width, is not, and says so at its own site.
