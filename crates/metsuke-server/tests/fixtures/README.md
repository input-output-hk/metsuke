...
- `get-unadorned` is an object with no metadata beside it, which only
  something other than this server writes.
- `put-refused` is a presigned PUT from a key the bucket grants nothing.
- `list-page-1..3` is the same corpus walked by continuation token, bounded to
...
- `list-after` is one bounded page from a cursor, which S3 reads exclusively.
