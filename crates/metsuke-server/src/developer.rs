...
pub enum AccountsError {
    /// Says where the parse stopped and nothing about what it read there.
    /// Every line of this file is a credential, so no message `toml` writes may
    /// reach the journal, which `systemd-journal` and anything shipping
    /// journals centrally read: it renders the line it failed on, and
    /// `message()` alone still renders the value for a number, a float or a
    /// bool, from the parser as well as from serde. The reason here is this
    /// variant's own text, which names the format the file has to be in.
    #[error("is not one `user = \"password\"` line per account, with the password quoted ({at})")]
    NotATable { at: String },
...
impl Accounts {
    ...
    pub fn parse(text: &str) -> Result<Accounts, AccountsError> {
        // Not `?`: nothing the parser wrote may travel, so only the line it
        // stopped on reaches the variant. Counting newlines rather than
        // slicing by line, because a span start is a byte offset and a `get`
        // that lands off a boundary answers None rather than panicking.
