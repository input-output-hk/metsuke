...
enum Unreadable {
    ...
    /// Refused rather than merged or taken last-one-wins. Two spellings of one
    /// pool cannot both be the chain's answer, which keys pool-state by the
    /// hash itself, so a file holding them is one this server cannot read the
    /// writer's intent from, and the intent decides whose submissions are
    /// accepted (ADR 0011). Merging would keep a broken writer working and
    /// silent, and the entry that lost is a pool that can no longer submit.
    #[error("names pool {pool_id} twice, under {first:?} and {again:?}")]
    PoolTwice {
        pool_id: PoolId,
        first: String,
        again: String,
...
/// Holding this lock is a compare and a string clone, so nothing under it can
/// panic and leave it poisoned.
const SAID_LOCK: &str = "the said lock is never poisoned: saying cannot panic";
...
pub struct Roster {
    ...
    /// The last failure already said, so the same one is not said again.
    ///
    /// A re-read is attempted per lookup, and a lookup happens per upload that
    /// names an allowlisted pool, which is checked before the signature and
    /// before the limiter (`intake::Intake::accept`, and it says why). A pool
    /// id is public on chain and a Leios key can be generated by anybody, so
    /// while the roster is unreadable every such body is one journal line from
    /// a stranger, ahead of everything that would slow them down.
    ///
    /// The attempt itself is not held back: it is two failing syscalls, and a
    /// window over it would keep a roster that came back from being picked up
    /// until the window was out.
    said: RwLock<Option<String>>,
}
...
impl Roster {
    ...
    fn refresh(&self) {
        ...
        match self.reread() {
            Ok(()) => {
                // Cleared first, so the next failure is said even if it is the
                // one said before this read succeeded.
                self.said.write().expect(SAID_LOCK).take();
                let (epoch, slot) = self.position();
                eprintln!(
            ...
            Err(error) => self.say_once(&error),
        }
    }

    /// A failure said the first time and then not again until it changes or a
    /// read succeeds. Which failure it is decides that, not how long ago it
    /// was: an operator watching this needs the reason and the recovery, and
    /// nothing in between adds one.
    fn say_once(&self, error: &RosterError) {
        let line = error.to_string();
        let mut said = self.said.write().expect(SAID_LOCK);
        if said.as_deref() == Some(line.as_str()) {
            return;
        }
        eprintln!("{WARNING}{line}; keeping the roster already loaded");
        *said = Some(line);
    }
    ...
    fn read_file(&self) -> Result<Loaded, Unreadable> {
        ...
        // What each pool id was spelled as, so a second spelling of one can
        // name the first in the refusal. The file's keys are hex the writer
        // transcribed, and `from_hex` normalises, so two of them reaching one
        // `PoolId` is a thing a `HashMap<String, _>` cannot rule out.
        let mut spelled: HashMap<PoolId, String> = HashMap::with_capacity(file.pools.len());
        for (hex, keys) in file.pools {
            let pool_id = PoolId::from_hex(&hex).map_err(|source| Unreadable::NotAPoolId {
                found: hex.clone(),
                source: Box::new(source),
            })?;
            if let Some(first) = spelled.insert(pool_id, hex.clone()) {
                return Err(Unreadable::PoolTwice {
                    pool_id,
                    first,
                    again: hex,
                });
